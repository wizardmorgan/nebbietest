...
for dir in players rent objects; do
  echo "Copia ${dir}/..."
  mkdir -p "${DEST_LIB}/${dir}"
  rsync -a --delete "${MONTERO_LIB}/${dir}/" "${DEST_LIB}/${dir}/"
done

echo "Copia myst.*..."
shopt -s nullglob
MYST_FILES=("${MONTERO_LIB}"/myst.*)
if [ ${#MYST_FILES[@]} -eq 0 ]; then
  echo "ERROR: nessun file myst.* in ${MONTERO_LIB}" >&2
  exit 1
fi
cp -a "${MYST_FILES[@]}" "${DEST_LIB}/"
