...
NebbieDash.HELP_TEXT = {
  ...
  { "nidentbatch [nome-toon]", "Identify batch: oload/stat/identify da CSV → un CSV risultati (solo Sirio)." },
...
function NebbieDash.ensureIdentBatchCommandsFile()
  ...
  if not f then return end
  f:write(
    "# Sequenza identify batch — una riga = un comando inviato al MUD.\n" ..
...
function NebbieDash.batchRunCurrentStep()
  ...
  local sendCmd, logLabel, waitMode = NebbieDash.batchPrepareCommand(template, row)
  b.stepLines = {}
...
function NebbieDash.startBatchJob(mode, filterStr, commands, filterLabel, startMsg)
  if NebbieDash.currentChar ~= NebbieDash.BATCH_EXECUTOR then
    local cmdName = (mode == "ident") and "nidentbatch" or "nbatch"
    cecho("<orange>[NebbieDash] " .. cmdName .. ": serve " .. NebbieDash.BATCH_EXECUTOR ..
