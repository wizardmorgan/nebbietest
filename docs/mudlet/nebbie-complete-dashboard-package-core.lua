...
-- Larghezza automatica (default): il pannello si allarga/restringe da solo
-- in base al contenuto piu' lungo attualmente visibile, cosi' non "sparisce"
-- niente oltre il bordo dello schermo. `nwidth <numero>` disattiva questa
-- modalita' e fissa una larghezza manuale; `nwidth auto` la riattiva.
NebbieDash.autoWidth = true
...
function NebbieDash.initGUI()
  ...
  -- Il pannello usa solo il bordo destro. Azzeriamo esplicitamente il bordo
  -- sinistro perche' un package precedente (o una sessione Mudlet precedente)
  -- potrebbe averlo impostato: i bordi sono un'impostazione del profilo, non
  -- di uno script specifico, quindi disinstallare un vecchio package non lo
  -- resetta da solo (bug segnalato: "colonna nera a sinistra" senza motivo).
  setBorderLeft(0)
...
-- Calcola quanti caratteri (colonna piu' larga) servono per mostrare tutto
-- il contenuto attuale senza andare a capo — titoli, equip (posizione +
-- descrizione troncata), spell attive (nome + tick) e speedwalk (descrizione
-- + anteprima direzioni troncata).
function NebbieDash.computeContentMaxChars()
  local maxChars = 20
  local name = NebbieDash.currentChar
  if name then
    maxChars = math.max(maxChars, #("Equip — " .. name), #("Spell attivi — " .. name))
    local data = NebbieDash.getCharData(name)
    for _, entry in pairs(data.eq or {}) do
      ...
      maxChars = math.max(maxChars, #location + 6, #item + 6)
    end
    for _, s in ipairs(data.spells or {}) do
      maxChars = math.max(maxChars, #(s.name or "") + #(" " .. tostring(s.ticks) .. " tick"))
    end
  end
...
-- Applica la larghezza automatica (se attiva) in base al contenuto corrente,
-- senza mai superare il 60% della larghezza reale della finestra di Mudlet —
-- cosi' il pannello non puo' mai "uscire" dallo schermo.
function NebbieDash.applyAutoWidth()
  if not NebbieDash.autoWidth or not NebbieDash._guiCreated then return end
  local maxChars = NebbieDash.computeContentMaxChars()
  local charW = calcFontSize(NebbieDash.fontSize)
  charW = charW or 8
  local w = math.floor(charW * (maxChars + 3))
  local mainW = select(1, getMainWindowSize()) or 1024
  w = math.max(NebbieDash.autoWidthMin, math.min(w, math.floor(mainW * 0.6)))
  if w ~= NebbieDash.guiWidth then
    NebbieDash.guiWidth = w
    setBorderRight(w)
  end
end
...
function NebbieDash.cmdSetWidth(widthStr)
  if (widthStr or ""):lower():match("^%s*auto%s*$") then
    NebbieDash.autoWidth = true
    NebbieDash.refreshDashboard()
    cecho("<green>[NebbieDash] Larghezza pannello: automatica (si adatta al contenuto, entro il 60% dello schermo).\n")
    return
  end
  local width = tonumber(widthStr)
  if not width or width < 150 or width > 900 then
    cecho("<orange>[NebbieDash] Uso: nwidth <numero tra 150 e 900> oppure <yellow>nwidth auto<orange> (attuale: "
      .. NebbieDash.guiWidth .. (NebbieDash.autoWidth and ", automatica" or ", manuale") .. ")\n")
    return
  end
  NebbieDash.autoWidth = false
  NebbieDash.guiWidth = width
  if NebbieDash._guiCreated then
    setBorderRight(width)
    NebbieDash.positionGUI()
  end
  cecho("<green>[NebbieDash] Larghezza pannello impostata a " .. width .. " (manuale — <yellow>nwidth auto<green> per tornare automatica).\n")
end
  ...
  if data.spells and #data.spells > 0 then
    ...
    for _, s in ipairs(data.spells) do
      ...
      cechoLink("NebbieDashSpells", color .. s.name,
        string.format("NebbieDash.cmdQuickCast(%q, %q, %q)", prefix, s.name, name),
        "Clicca per rilanciare su " .. name .. ": " .. s.name, true)
