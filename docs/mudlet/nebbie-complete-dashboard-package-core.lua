...
function NebbieDash.computeEquipMaxChars(data)
  ...
  if data and data.eqUpdated then
    for _, row in ipairs(NebbieDash.buildEquipRows(data)) do
      if row.empty then
        maxChars = math.max(maxChars, #row.location + 9) -- " (vuoto)"
      else
        local item = NebbieDash.truncate(row.item or "?", NebbieDash.itemMaxLen)
        maxChars = math.max(maxChars, #row.location, #item + 5)
      end
    end
  end
  return maxChars
...
function NebbieDash.refreshDashboard()
  ...
  if not (data.eqUpdated) then
    ...
    -- buildEquipRows(). Il numero di slot del gioco non viene piu' mostrato:
    -- e' solo un contatore progressivo, non un identificatore di posizione.
    for _, row in ipairs(NebbieDash.buildEquipRows(data)) do
      if row.empty then
        cecho("NebbieDashEquip", string.format("<grey>%s (vuoto)\n", row.location))
      else
        local item = NebbieDash.truncate(row.item or "?", NebbieDash.itemMaxLen)
        cecho("NebbieDashEquip", string.format("<white>%s\n     <green>%s\n", row.location, item))
      end
    end
  end
